...
ContentPage {
    ...
    SettingsCardSection {
        ...
        expanded: false
        icon: "view_sidebar"
        title: Translation.tr("Widget Bar")

        SettingsGroup {
            SettingsSwitch {
                buttonIcon: "toggle_on"
                text: Translation.tr("Enable Widget Bar")
                checked: Config.options?.widgetBar?.enable ?? true
                onCheckedChanged: {
                    Config.setNestedValue('widgetBar.enable', checked)
                }
                StyledToolTip {
                    text: Translation.tr("Show a dedicated widget panel with media, weather, calendar, and quick navigation")
                }
            }

            ConfigSelectionArray {
                Layout.fillWidth: false
                currentValue: Config.options?.widgetBar?.position ?? "left"
                onSelected: newValue => {
                    Config.setNestedValue('widgetBar.position', newValue)
                }
                options: [
            ...
            ConfigSpinBox {
                ...
                value: Config.options?.widgetBar?.width ?? 320
                from: 200
                to: 500
                stepSize: 20
                onValueChanged: {
                    Config.setNestedValue('widgetBar.width', value)
                }
            }
            ...
            ContentSubsection {
                ...
                SettingsSwitch {
                    ...
                    checked: Config.options?.widgetBar?.showMedia ?? true
                    onCheckedChanged: {
                        Config.setNestedValue('widgetBar.showMedia', checked)
                    }
                }
                ...
                SettingsSwitch {
                    ...
                    checked: Config.options?.widgetBar?.showWeather ?? true
                    onCheckedChanged: {
                        Config.setNestedValue('widgetBar.showWeather', checked)
                    }
                }
                ...
                SettingsSwitch {
                    ...
                    checked: Config.options?.widgetBar?.showCalendar ?? true
                    onCheckedChanged: {
                        Config.setNestedValue('widgetBar.showCalendar', checked)
                    }
                }

                SettingsSwitch {
                    buttonIcon: "apps"
                    text: Translation.tr("Quick navigation")
                    checked: Config.options?.widgetBar?.showQuickNav ?? true
                    onCheckedChanged: {
                        Config.setNestedValue('widgetBar.showQuickNav', checked)
                    }
                }
            }
        }
